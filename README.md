...
| **> ../deliverables/sbom_syft_spdx.json**| Standard shell redirection (*>*) that save the previous line's results in a file named *sbom_syft_spdx.json* in the `deliverables` folder, which is one directory level up from your current folder
...
| **Syntax Breakdown** | **Description**
|---------------|------------------------|
| **grype** | Open-source program used to scan something for vulnerabilities
| **sbom:deliverables/sbom_syft_spdx.json** | Program sub-command (*sbom*) telling Grype to use an existing SBOM as its input instead of scanning files directly
...
| **> ../deliverables/vuln_analysis_grype.txt**| Standard shell redirection (*>*) that takes the previous command and saves it in a file named *vuln_analysis_grype.txt* in the `deliverables` folder, one level up from the current directory
